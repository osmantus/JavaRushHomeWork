    ...
    public Sayable toSayable(final int i) {
        ...
        class DogPet extends SuperDog implements Sayable
        {
            ...
            public String say()
            {
                ...
                if (i <= 0)
                {
                    result = getName().concat(" спит.");
                    return result;
                }
                else
                {
                    result = getName().concat(" лает г");
                    for (int j = 0; j < i; j++)
                        result = result.concat("а");
                    result = result.concat("в!");
                    ...
                    result = result.concat(formatter.format(today.getTime()));
                    return result;
                }
            }
            ...
            {
                return this.getSuperQuotes().concat(name).concat(getSuperQuotes());
            }
        }
