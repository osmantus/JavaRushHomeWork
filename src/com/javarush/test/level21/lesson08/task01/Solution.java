...
public class Solution implements Cloneable {
    ...
    {
        Solution copySol = null;
        try
        {
            copySol = new Solution();
            copySol.users = new LinkedHashMap<String, User>();
            User user = null;
            for (Map.Entry<String, User> eachUser : users.entrySet())
            {
                user = new User(eachUser.getValue().getAge(), eachUser.getValue().getName());
                copySol.users.put(eachUser.getKey(), user);
            }
        }
        catch (Exception e)
