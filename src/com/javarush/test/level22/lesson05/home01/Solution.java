        ...
        if (string != null && threadName != null)
        {
            if (string.contains("\t"))
            {
                ...
                while (subStr.indexOf((int)'\t') >= 0)
                {
                    if (index >= 1)
                    {
                        ...
                        {
                            if (subStr.contains("\t"))
                                result = result.concat("\t").concat(subStr.substring(0, subStr.indexOf('\t')));
                            else
                                result = result.concat("\t").concat(subStr);
                        } else
                        {
                            if (subStr.contains("\t"))
            ...
            } else
            {
                if (threadName.equals(FIRST_THREAD_NAME))
                    throw new TooShortStringFirstThreadException(new java.lang.StringIndexOutOfBoundsException(-1));
                else if (threadName.equals(SECOND_THREAD_NAME))
                    throw new TooShortStringSecondThreadException(new java.lang.StringIndexOutOfBoundsException(-1));
                else
                    throw new RuntimeException(new java.lang.StringIndexOutOfBoundsException(-1));
            }
        }
        else
        {
