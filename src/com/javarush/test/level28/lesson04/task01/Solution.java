...
public class Solution {
    ...
    {
        private static AtomicInteger factoryID;
        private static volatile HashMap<Integer, AtomicInteger> factoryByID;
        private AtomicInteger threadCount = new AtomicInteger(1);

        static {
            if (factoryID == null)
                factoryID = new AtomicInteger(1);
            if (factoryByID == null)
                factoryByID = new HashMap<>();
        }

        public AmigoThreadFactory()
        {
            factoryByID.put(this.hashCode(), new AtomicInteger(factoryID.get()));
            factoryID.incrementAndGet();
        }
        ...
        public Thread newThread(Runnable r)
        {
            ...
            String threadSubGroupA = String.valueOf(factoryByID.get(this.hashCode()));

            //AtomicInteger threadID = new AtomicInteger(1);

            String threadSubGroupB = String.valueOf(threadCount);

            newThreadName = threadGroup.concat("-pool-").concat(threadSubGroupA).concat("-thread-").concat(threadSubGroupB);
